...
def cli_user(app: Flask):
    ...
    def cli_user_bulk_create(quantity):
        ...
        fake = Faker()
        users_to_insert = (UserModel(nickname=fake.name(), email=fake.unique.email()) for _ in range(int(quantity)))
...
def cli_post(app: Flask):
    ...
    def cli_post_bulk_create(quantity, user_scope):
        ...
        users_in_chunk: List[UserModel] = (
            session.query(UserModel).filter(and_(UserModel.id >= n_range[0], UserModel.id <= n_range[1])).all()
        )

        user_post_list = lambda: (PostModel(title=fake.sentence(), content=fake.text()) for _ in range(int(quantity)))

        for user in users_in_chunk:
